...
class InstallerSurfaceTests(unittest.TestCase):
    ...
    def test_windows_installer_hands_runtime_over_to_installed_cli(self) -> None:
        ...
        assert_matches(self, wxs, r'FileRef="RunInstallCmd"')
        assert_matches(self, wxs, r'ExeCommand="-Cleanup"')
    ...
    def test_versions_stay_aligned_for_next_release(self) -> None:
        ...
        expected_version = subprocess.check_output(
            ["python3", "scripts/read_release_version.py"],
            cwd=ROOT,
            text=True,
        ).strip()

        self.assertEqual(workspace["workspace"]["package"]["version"], expected_version)
        self.assertEqual(tray_cargo["package"]["version"], expected_version)
    ...
    def test_macos_tray_updater_is_wired_for_signed_in_app_updates(self) -> None:
        ...
        assert_matches(self, updater_rs, r'TRAPEZOHE_UPDATER_ENDPOINT')
        assert_matches(self, updater_rs, r"effective_updater_endpoint")
        assert_matches(self, updater_rs, r"updater_builder\(\)\.endpoints")
        ...
        assert_matches(self, home_page, r"isUpdateActionLocked")
    ...
    def test_release_workflow_publishes_macos_updater_archive_signature_and_manifest(self) -> None:
        ...
        assert_matches(self, updater_script, r'TRAPEZOHE_UPDATER_BASE_URL:-')
        ...
        assert_matches(self, workflow, r"Resolve updater mirror settings")
        assert_matches(self, workflow, r'TRAPEZOHE_UPDATER_ENDPOINT=')
        assert_matches(self, workflow, r'TRAPEZOHE_UPDATER_BASE_URL=')
        assert_matches(self, workflow, r"CLOUDFLARE_R2_PUBLIC_BASE_URL")
        assert_matches(self, workflow, r"CLOUDFLARE_R2_BUCKET")
        assert_matches(self, workflow, r"Sync release artifacts to Cloudflare R2")
        assert_matches(self, workflow, r"CLOUDFLARE_API_TOKEN")
        assert_not_matches(self, workflow, r"CLOUDFLARE_OAUTH_TOKEN")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            subprocess.run(
                [
                    ...
                    str(out_path),
                    "--fallback-root",
                    "https://github.com/Trapezohe/companion_service/releases/download/v0.1.24",
                    "--release-page",
                    "https://github.com/Trapezohe/companion_service/releases/tag/v0.1.24",
                    "--primary-root",
                    "https://pub-6c87d0ddc0de485caf0c5695e4b922db.r2.dev/v0.1.24",
                ],
                check=True,
                cwd=ROOT,
            )
            ...
            self.assertEqual(
                ...
                "https://pub-6c87d0ddc0de485caf0c5695e4b922db.r2.dev/v0.1.24/trapezohe-companion-macos.pkg",
            )
            self.assertEqual(
                manifest["platforms"]["macos"]["fallback_url"],
                "https://github.com/Trapezohe/companion_service/releases/download/v0.1.24/trapezohe-companion-macos.pkg",
            )
            self.assertEqual(manifest["platforms"]["macos"]["sha256"], "macdigest")
            self.assertEqual(
                manifest["platforms"]["windows"]["primary_url"],
                "https://pub-6c87d0ddc0de485caf0c5695e4b922db.r2.dev/v0.1.24/trapezohe-companion-windows.msi",
            )
    ...
    def test_windows_build_scripts_no_longer_shell_out_to_node_for_plan_generation(self) -> None:
        ...
        assert_matches(self, tray_script, r'@\("build", "--manifest-path", \$TrayManifestPath, "--release", "--features", "custom-protocol"\)')
    ...
    def test_manual_windows_test_workflow_builds_msi_on_windows_runner(self) -> None:
        workflow = read(".github/workflows/windows-test-msi.yml")

        assert_matches(self, workflow, r"name:\s+Windows Test MSI")
        assert_matches(self, workflow, r"workflow_dispatch:")
        assert_matches(self, workflow, r"artifact_suffix:")
        assert_matches(self, workflow, r"test-surface:")
        assert_matches(self, workflow, r"runs-on:\s+ubuntu-latest")
        assert_matches(self, workflow, r"python3 -m unittest discover -s tests -p '\*_test\.py'")
        assert_matches(self, workflow, r"runs-on:\s+windows-latest")
        assert_matches(self, workflow, r"uses:\s+actions/checkout@v4")
        assert_matches(self, workflow, r'python \./scripts/read_release_version\.py')
        assert_matches(self, workflow, r"uses:\s+actions/setup-node@v4")
        assert_matches(self, workflow, r'node-version:\s+"20"')
        assert_matches(self, workflow, r"uses:\s+actions/setup-dotnet@v4")
        assert_matches(self, workflow, r'uses:\s+dtolnay/rust-toolchain@stable')
        assert_matches(self, workflow, r'cargo test --manifest-path tray/Cargo\.toml --locked')
        assert_matches(self, workflow, r'\./scripts/build-windows-msi\.ps1 -Version')
        assert_matches(self, workflow, r'Get-FileHash \.\\dist\\installers\\trapezohe-companion-windows\.msi -Algorithm SHA256')
        assert_matches(self, workflow, r"uses:\s+actions/upload-artifact@v4")
        assert_matches(self, workflow, r"retention-days:\s+7")
