...
pub struct PermissionsState {
    /// Companion-side enable/disable flags, keyed by permission id
    companion_flags: std::collections::HashMap<String, bool>,
}

impl PermissionsState {
    pub fn new() -> Self {
        Self {
            companion_flags: std::collections::HashMap::new(),
        }
    }

    pub fn is_enabled(&self, id: &str) -> bool {
        self.companion_flags.get(id).copied().unwrap_or(false)
    }

    pub fn set_enabled(&mut self, id: &str, enabled: bool) {
        self.companion_flags.insert(id.to_string(), enabled);
    }
}
...
pub fn build_permissions_snapshot(state: &Mutex<PermissionsState>) -> PermissionsSnapshot {
    let flags = state.lock().unwrap();
...
fn build_system_permission(id: &str, flags: &PermissionsState) -> PermissionItem {
    ...
    PermissionItem {
        ...
        system_auth,
        companion_enabled: flags.is_enabled(id),
        is_high_risk: false,
...
mod tests {
    ...
    fn test_default_permissions_all_disabled() {
        let state = Mutex::new(PermissionsState::new());
        let snapshot = build_permissions_snapshot(&state);
    ...
    fn test_toggle_companion_flag() {
        let state = Mutex::new(PermissionsState::new());
        state.lock().unwrap().set_enabled("local_command", true);
        let snapshot = build_permissions_snapshot(&state);
