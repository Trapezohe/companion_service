...
test('macOS tray updater is wired for signed in-app updates with a manual-install fallback for unsupported app locations', () => {
  ...
  const homePage = read('tray/ui-react/src/components/panel/HomePage.tsx')
  const settingsPage = read('tray/ui-react/src/components/panel/SettingsPage.tsx')
  ...
  assert.match(homePage, /useCheckUpdate, useInstallUpdate/)
  assert.match(homePage, /update\.available && update\.can_install && isVersionNewer\(update\.latest_version, update\.current_version\)/)
  assert.match(homePage, /updateManualInstall/)
  assert.match(homePage, /updateReady/)
  assert.match(settingsPage, /useOpenReleasePage/)
  assert.match(settingsPage, /openReleasePage\.mutate\(\)/)
...
test('tray panel surface is a narrow React dashboard with built-in language switching', () => {
  ...
  const panel = read('tray/ui-react/src/components/panel/CompanionPanel.tsx')
  const homePage = read('tray/ui-react/src/components/panel/HomePage.tsx')
  const settingsPage = read('tray/ui-react/src/components/panel/SettingsPage.tsx')
  const logsPage = read('tray/ui-react/src/components/panel/LogsPage.tsx')
  const viteConfig = read('tray/ui-react/vite.config.ts')

  assert.match(app, /CompanionPanel/)
  assert.match(panel, /HomePage/)
  assert.match(panel, /PermissionsPage/)
  assert.match(panel, /LogsPage/)
  assert.match(panel, /SettingsPage/)
  assert.match(viteConfig, /base:\s*command === 'serve' \? '\/' : '\.\/'/)
  assert.match(panel, /点面板外即可关闭/)
  assert.match(homePage, /MenuRow/)
  assert.match(homePage, /navPermissions/)
  assert.match(homePage, /navLogs/)
  assert.match(homePage, /navSettings/)
  assert.match(settingsPage, /setLanguage\.mutate\('en'\)/)
  assert.match(settingsPage, /setLanguage\.mutate\('zh'\)/)
  assert.match(logsPage, /action_logs/)
})
...
test('tray panel exposes a release-page fallback and stable MCP status helpers from React components', () => {
  ...
  assert.match(settingsPage, /openReleasePage\.mutate\(\)/)
  assert.match(homePage, /function serverStatusLabel\(/)
  assert.match(homePage, /function serverStatusPillClass\(/)
  assert.match(homePage, /diagnostics\?\.servers/)
})
...
test('tray panel uses a dark anchored dropdown surface in the React frontend', () => {
  ...
  assert.match(panelCss, /--color-panel:\s*rgba\(32,\s*32,\s*34,\s*0\.98\)/i)
  assert.match(panel, /w-\[360px\]/)
  assert.match(panel, /max-h-\[640px\]/)
  assert.match(panel, /setSize\(new LogicalSize\(PANEL_WIDTH, nextHeight\)\)/)
  assert.equal(tauriConfig.app?.windows?.[0]?.width, 360)
...
test('tray panel keeps service actions in settings, keeps version info on overview, and avoids latest-version noise', () => {
  ...
  assert.match(homePage, /onNavigate\('logs'\)/)
  assert.match(homePage, /onNavigate\('settings'\)/)
  assert.match(homePage, /versionFooter/)
  assert.match(homePage, /updateNote/)
  assert.doesNotMatch(homePage, /upToDate/)
  assert.match(settingsPage, /stopService\.mutate\(\)/)
  assert.match(settingsPage, /quitTray\.mutate\(\)/)
})
...
test('React overview exposes self-check repair actions and the admin confirm dialog executes sensitive repairs', () => {
  ...
  assert.match(homePage, /self_check/)
  assert.match(homePage, /repair_actions/)
  assert.match(homePage, /showAdminConfirm\(/)
  assert.match(homePage, /register_native_host/)
  assert.match(homePage, /runRepair\.mutate\(action\.id\)/)
